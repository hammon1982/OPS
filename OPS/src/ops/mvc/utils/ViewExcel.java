...
public class ViewExcel extends AbstractExcelView {
	...
	public void buildExcelDocument(Map model, HSSFWorkbook workbook,
		...
		// 产生标题列
		header.createCell(0, HSSFCell.CELL_TYPE_STRING).setCellValue("ID");
		header.createCell(1, HSSFCell.CELL_TYPE_STRING).setCellValue("名称");
		...
		// 列总和计算
		/*
		 * HSSFRow row = sheet.createRow(rowNum); row.createCell((short)
		 * 0).setCellValue("TOTAL:"); String formual = "SUM(D2:D" + rowNum +
		 * ")"; // D2到D[rowNum]单元格起(count数据) row.createCell((short)
		 * 3).setCellFormula(formual);
		 */
